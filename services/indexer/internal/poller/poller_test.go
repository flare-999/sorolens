...
func (f *fakeStore) BatchInsertWithCursor(ctx context.Context, network string, ledger uint32, events []Event, invocations []Invocation, syncState SyncState) error {
	...
	// A real transaction aborts the whole batch if any row is rejected, which
	// is what makes the DLQ isolation path reachable.
	for _, e := range events {
		if f.eventInsertErrs != nil {
			if err, ok := f.eventInsertErrs[e.ID]; ok {
				return err
...
// buildCodeEntryXDR encodes a CONTRACT_CODE LedgerEntry for wasmHash carrying
// code, mirroring what the RPC returns for wasm.ContractCodeKey.
